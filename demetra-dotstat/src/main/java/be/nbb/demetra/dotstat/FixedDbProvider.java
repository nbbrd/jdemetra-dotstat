 ...
 * @param <BEAN>
 */
abstract class FixedDbProvider<BEAN extends FixedDbBean> extends AbstractDataSourceLoader<DbAccessor<BEAN>, BEAN> {

    private static final IParam<DataSet, Dims> DIM_MAP = new DimsParam();

    public FixedDbProvider(Logger logger, String providerName, TsAsyncMode asyncMode) {
        super(logger, providerName, asyncMode);
    }

    @Override
    public String getDisplayName(DataSource dataSource) {
        FixedDbBean bean = decodeBean(dataSource);
    ...
    public List<DataSet> children(DataSource dataSource) throws IOException {
        ...
        Dims dims = new Dims(FixedDbBean.getDimArray(dataSource));
    ...
    private static final class DimsParam implements IParam<DataSet, Dims> {
        ...
        public Dims get(DataSet dataSet) {
            String[] dimColumns = FixedDbBean.getDimArray(dataSet.getDataSource());
