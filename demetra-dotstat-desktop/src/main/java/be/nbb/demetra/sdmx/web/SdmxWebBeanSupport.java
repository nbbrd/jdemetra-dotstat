...
class SdmxWebBeanSupport {
    ...
    @NbBundle.Messages({"bean.cache.description=Mechanism used to improve performance."})
    public static Sheet newSheet(SdmxWebBean bean, SdmxWebProvider provider) {
        ConcurrentMap<Object, Object> autoCompletionCache =
                GuavaCaches.ttlCacheAsMap(Duration.ofMinutes(1));
        ...
        result.put(
                withCache(b.reset("Cache").description(Bundle.bean_cache_description()), bean)
                        .build());
        return result;
    }
    ...
    private static NodePropertySetBuilder withSource(
            NodePropertySetBuilder b,
            SdmxWebBean bean,
        ...
        SdmxAutoCompletion database =
                SdmxAutoCompletion.onDatabase(provider, bean, autoCompletionCache);
        ...
        SdmxAutoCompletion dataflow =
                SdmxAutoCompletion.onFlow(provider, bean, autoCompletionCache);
    ...
    private static NodePropertySetBuilder withOptions(
            NodePropertySetBuilder b,
            SdmxWebBean bean,
            SdmxWebProvider provider,
            ConcurrentMap<Object, Object> autoCompletionCache) {
        SdmxAutoCompletion dimension =
                SdmxAutoCompletion.onDimension(provider, bean, autoCompletionCache);
        ...
        b.withAutoCompletion()
                ...
                .defaultValueSupplier(
                        () ->
                                dimension.getSource().getValues("").stream()
                                        .map(sdmxdl.Dimension.class::cast)
                                        .map(Dimension::getId)
        ...
        SdmxAutoCompletion attribute =
                SdmxAutoCompletion.onAttribute(provider, bean, autoCompletionCache);
